...
namespace Libary_Manager.Libary_GUI
{
    public partial class Libary_QuanLy : Form
    {
        ...
        private void TcLibaryQuanLy_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            else if (selectedTabPage == TabChiNhanh)
            {
                TabChiNhanhAction();
            }
        }
        ...
        private void DgvSachThuVien_CellClick(object sender, DataGridViewCellEventArgs e)
        {
            if (e.RowIndex >= 0 && e.ColumnIndex >= 0)
            {
                ...
                string maSachValue = row.Cells["maSach"].Value.ToString();
                string chiNhanhValue = row.Cells["chiNhanh"].Value.ToString();
                
                if (maSachValue != null)
                {
                    CbbChiNhanh.Text = chiNhanhValue;
                    sachDTO.maSach = maSachValue.ToString();
                }
            }
        }
